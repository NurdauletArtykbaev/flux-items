...
//            $query = $query->where('items.got_n8n', false)->orderBy('items.id');
    ...
    public function getNewDataProducts($filters = [], $relations = ['images', 'rentTypes.pivot.prices', 'user'], $exists = ['images'])
    {
        $user = auth()->guard('sanctum')->user();
        $filters['exists'] = $exists;
...
//            $query = $query->where('items.got_n8n', false)->orderBy('items.id');
    ...
    public function getPaginationTestProducts($filters = [], $relations = ['images', 'rentTypes.pivot.prices', 'user'],
